...
if (!requireNamespace("ggplot2", quietly = TRUE)) {
  stop(
    "Package 'ggplot2' is required for Ne_sensitivity functions.\n",
...
.resolve_stage <- function(stage_index, stage_name, s, T_mat) {
  ...
  col_names <- if (!is.null(colnames(T_mat))) {
    colnames(T_mat)
  } else {
  ...
  if (!is.null(stage_index)) {
    if (
      !is.numeric(stage_index) ||
        length(stage_index) != 1 ||
        stage_index < 1 ||
        stage_index > s
    ) {
      stop(paste0(
        "stage_index must be a single integer between 1 and ",
        s,
        "."
      ))
    }
  ...
  if (!is.null(stage_name)) {
    ...
    if (is.na(idx)) {
      stop(paste0(
        "stage_name '",
        stage_name,
        "' not found in column names of T_mat.\n",
        "  Available names: ",
...
Ne_sensitivity_Vk <- function(
  ...
  if (grepl("clonal", fn_name)) {
    stop(
      "Ne_sensitivity_Vk does not apply to Ne_clonal_Y2000.\n",
  ...
  results <- lapply(Vk_range, function(vk) {
    ...
    if (!is.null(r$V_term1)) {
      row$V_term1 <- r$V_term1
    }
    if (!is.null(r$V_term2)) {
      row$V_term2 <- r$V_term2
    }
    if (!is.null(r$V_term3)) {
      row$V_term3 <- r$V_term3
    }
    if (!is.null(r$V_component1)) {
      row$V_component1 <- r$V_component1
    }
    if (!is.null(r$V_component2)) {
      row$V_component2 <- r$V_component2
    }
    row
  })
  ...
  p <- ggplot2::ggplot(df, ggplot2::aes(x = Vk_over_k, y = NeN)) +
    ...
    ggplot2::labs(
      ...
      x = expression(paste(
        "Variance-to-mean ratio of sexual output  ",
        (V[k] / bar(k))[i]
      )),
      y = expression(N[e] / N),
      caption = paste0(
        "Other stages fixed at Vk/k_bar = ",
        Vk_fixed,
...
Ne_sensitivity_Vc <- function(
  ...
  p <- ggplot2::ggplot(df, ggplot2::aes(x = Vc_over_c, y = NeN)) +
    ...
    ggplot2::labs(
      ...
      x = expression(paste(
        "Variance-to-mean ratio of clonal output  ",
        (V[c] / bar(c))[i]
      )),
      y = expression(N[e] / N),
      caption = paste0(
        "Other stages fixed at Vc/c_bar = ",
        Vc_fixed,
...
Ne_sensitivity_d <- function(
  ...
  if (!is.numeric(d_fixed) || length(d_fixed) != s) {
    stop(paste0(
      "d_fixed must be a numeric vector of length ",
      s,
  ...
  p <- ggplot2::ggplot(df, ggplot2::aes(x = d_focal, y = NeN)) +
    ...
    ggplot2::labs(
      ...
      y = expression(N[e] / N),
      caption = paste0(
        "Other stage d values fixed. Dashed line = base d value (",
        d_original,
        ").\n",
...
Ne_sensitivity_L <- function(
  ...
  results <- lapply(L_range, function(lval) {
    ...
    if (grepl("sexual|mixed", fn_name)) {
      args$Vk_over_k <- Vk_over_k
    }
  ...
  model_label <- if (grepl("clonal", fn_name)) {
    "Clonal model (Eq. 10)"
  } else if (grepl("sexual", fn_name)) {
    "Sexual model (Eq. 6, d=0)"
  } else if (grepl("mixed", fn_name)) {
    "Mixed model (Eq. 6)"
  } else {
    fn_name
  }

  line_color <- if (grepl("clonal", fn_name)) {
    "#d6604d"
  } else if (grepl("sexual", fn_name)) {
    "#2166ac"
  } else {
  ...
  p <- ggplot2::ggplot(df, ggplot2::aes(x = L, y = NeN)) +
    ...
    ggplot2::labs(
      ...
      y = expression(N[e] / N),
      caption = paste0(
        "V is held constant across L values.\n",
...
print.Ne_sensitivity <- function(x, ...) {
  ...
  # ------------------------------------------------------------------
  max_NeN <- max(df$NeN)
  max_idx <- which.max(df$NeN)
  min_NeN <- min(df$NeN)
  min_idx <- which.min(df$NeN)
  ...
  cat("  --- Ne/N summary ---\n")
  cat(sprintf(
    "    At reference (%s = %s)  : Ne/N = %.3f  |  Min N = %s\n",
    col,
    round(ref_val, 3),
    ref_NeN,
  ...
  cat(sprintf(
    "    Maximum Ne/N  (%s = %s)  : Ne/N = %.3f  |  Min N = %s\n",
    col,
    round(max_par, 3),
    max_NeN,
  ...
  cat(sprintf(
    "    Minimum Ne/N  (%s = %s)  : Ne/N = %.3f  |  Min N = %s\n",
    col,
    round(min_par, 3),
    min_NeN,
  ...
  cat(sprintf(
    "    Local elasticity at reference point : E = %s\n",
    if (is.na(E_local)) "undefined" else round(E_local, 3)
  ))
  cat(sprintf(
    "    Global elasticity (mean over range) : E = %s\n",
  ...
  if (!is.na(E_local)) {
    ...
    cat(sprintf(
      "      Direction: %s\n",
      if (E_local < 0) {
        "NEGATIVE -- higher variance reduces Ne/N (typical)"
      } else {
        "POSITIVE -- higher value increases Ne/N (atypical)"
      }
    ))
    cat("\n")
  }
  ...
  cat(sprintf(
    "    Min N at reference point             : %s\n",
    format(ref_MinN, big.mark = ",")
  ))
  cat(sprintf(
    "    Min N at worst case (%s = %s)  : %s\n",
    col,
    round(worst_par, 3),
    format(worst_MinN, big.mark = ",")
  ))
  cat(sprintf(
    "    Ratio worst/reference                : %.1fx more individuals\n",
    cons_ratio
  ))
  cat(sprintf(
    "    Ne target used                       : %s\n",
  ...
  cat(bench_msg, "\n")
  cat(sprintf(
    "    Frankham (1995) empirical range: %.2f -- %.2f\n",
    frankham_range[1],
    frankham_range[2]
  ))
  cat("\n")
